...
def temporal_stacked_to_separate(feature: Tensor, durations: List) -> Tensor:
    ...
    out_feature = torch.stack([F.pad(f, padding_value + [0, max_seq_len-len(f)]) for f in feature_splits])    # [b, t, *]
    return out_feature
...
def temporal_separate_to_stack(feature: Tensor, durations: List) -> Tensor:
    ...
    """
    out_feature = torch.cat([feature[i][:durations[i]] for i in range(len(feature))], dim=0) # [\sigma t_i *]
    return out_feature
...
def generate_anchor_scores(proposals, label, seq_len, thres_score):
    ...
    """
    illegal = torch.logical_or(proposals[..., 0] < 0, proposals[..., 1] >= seq_len)
...
def calculate_IoU_batch_temporal(box0: Tensor, box1: Tensor) -> Tensor:
    ...
    Return:
        iou: [b, n_boxes]
    """
    union = (torch.min(torch.stack([box0[..., 0], box1[..., 0]], 0), 0)[0], torch.max(torch.stack([box0[..., 1], box1[..., 1]], 0), 0)[0])
    inter = (torch.max(torch.stack([box0[..., 0], box1[..., 0]], 0), 0)[0], torch.min(torch.stack([box0[..., 1], box1[..., 1]], 0), 0)[0])
...
def generate_2d_gaussian(boxes, w, h, delta=0.05):
    ...
    gaussian = torch.exp(-(boxes[:, 0]-grid[:, 0])**2/(delta*boxes[:, 2]**2))*\
                torch.exp(-(boxes[:, 1]-grid[:, 1])**2/(delta*boxes[:, 3]**2))  # [k, h, w]
