...
from models.module.decompose import DynamicDecomposeBlock
from models.module.TCN import TCN
from .module.decoder import TemporalDecoderAnchor, TemporalDecoderRegression, TemporalDecoderDistribution, SpatialDecoder1D, SpatialDecoder2D
import numpy as np
...
class VideoEncoder(nn.Module):
    ...
    def forward(self, frames, frame_masks):
        frames = frames.chunk(4, dim=0)  # split videos to reduce GPU usage
        video_feature = []
        for frame in frames:
...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        self.reftrans = TCN(cfg.hidden_dim, cfg.hidden_dim, cfg.dim_feedforward,
                            cfg.hidden_dim, cfg.layers, dropout=cfg.dropout, decompose=self.cfg.att_decompose, decompose_type=cfg.decompose_type)

        if not cfg.decompose:
            self.spatial_decoder = SpatialDecoder2D(
                cfg.hidden_dim, cfg.spatial_decoder_hidden_dim, cfg.dilation)
        else:
            self.decompose = DynamicDecomposeBlock(cfg.hidden_dim, decompose_type=cfg.decompose_type)
            self.spatial_decoder = SpatialDecoder1D(
                cfg.hidden_dim, cfg.spatial_decoder_hidden_dim, cfg.dilation)

        if cfg.temporal_decoder_type == 'anchor':
            self.temporal_decoder = TemporalDecoderAnchor(
                cfg.hidden_dim, cfg.temporal_window_width, cfg.dropout, not_obj=cfg.not_decoder_obj)
        elif cfg.temporal_decoder_type == 'regression':
            self.temporal_decoder = TemporalDecoderRegression(
                cfg.hidden_dim, cfg.dropout, not_obj=cfg.not_decoder_obj)
        elif cfg.temporal_decoder_type == 'distribution':
            self.temporal_decoder = TemporalDecoderDistribution(
                cfg.hidden_dim, cfg.dropout, not_obj=cfg.not_decoder_obj)
        else:
    ...
    def forward(self, samples, captions, durations):  # [current]
        ...
        feature, maps_spatial = self.reftrans(
        ...
        if not self.cfg.decompose:
            feature_stack = temporal_separate_to_stack(feature.transpose(
                1, 2), durations)  # [\sigma t_i, c, h, w]
            spatial_result = self.spatial_decoder(feature_stack)
            ...
            feature_obj = ((spatial_hm * feature).sum((-2, -1)) /
                           spatial_hm.sum((-2, -1)) + 1e-5)  # [b, c, t]

        else:
            feature_obj, fea_th, fea_tw, A_obj = self.decompose(
                    feature, feature_masks)
            A_obj_stack = temporal_separate_to_stack(
                    A_obj.transpose(1, 2), durations)
            maps_spatial.append(A_obj_stack)
            fea_th_stack = temporal_separate_to_stack(
                fea_th.transpose(1, 2), durations)
            fea_tw_stack = temporal_separate_to_stack(
                fea_tw.transpose(1, 2), durations)
            spatial_result = self.spatial_decoder(
                fea_th_stack, fea_tw_stack)
...
def build_model(cfg):
    ...
    if cfg.temporal_decoder_type == 'anchor':
        ...
        weight_dict.update(
            {
                "temporal_dist_loss": cfg.loss_temporal_dist
            }
        )
