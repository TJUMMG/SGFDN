...
def train_one_epoch(
    ...
    for i, batch_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        outputs['maps_spatial'] = [m[keep] for m in outputs['maps_spatial']]
        ...
        time_mask = torch.ones(b, max_duration).bool().to(device)   # [b, t] "True" represent the padded moment
        ...
        loss_dict = {}
        if criterion is not None:
            loss_dict_cur = criterion(
                outputs, durations, inter_idx, targets, time_mask)
...
def evaluate(
    ...
    for i_batch, batch_dict in enumerate(metric_logger.log_every(data_loader, 100, header)):
        samples = batch_dict["samples"].to(device)
        durations = batch_dict["durations"]
        captions = batch_dict["captions"]
        targets = batch_dict["targets"]
        ...
        if args.test:
            ...
            maps_all = [m[keep] for m in outputs['maps_spatial']]
        outputs["spatial_map"] = outputs["spatial_map"][keep]
        outputs["spatial_wh"] = outputs["spatial_wh"][keep]
        outputs['maps_spatial'] = [m[keep] for m in outputs['maps_spatial']]
        b = len(durations)
        ...
        if criterion is not None:
            loss_dict.update(
                criterion(outputs, durations, inter_idx, targets, time_mask))
