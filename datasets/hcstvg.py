...
class VideoModulatedSTGrounding(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cap.set(cv2.CAP_PROP_POS_FRAMES, frame_ids[0])  # decoding from the first selected frame
        idx = frame_ids[0]
        images_list = []
...
def build(image_set, args):
    ...
    if args.test or image_set == "val" or image_set == "test":
        if not args.v2:  # only a test set
            ann_file = Path(args.hcstvg_ann_path) / f"test.json"
        else:  # only a val set
            ann_file = Path(args.hcstvg_ann_path) / f"val.json"
    else:
        ann_file = Path(args.hcstvg_ann_path) / f"train.json"
